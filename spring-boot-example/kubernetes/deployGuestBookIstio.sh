...
kubectl apply -f <(istioctl kube-inject -f $SCRIPTDIR/helloworld-deployment-v1.yaml)  -f $SCRIPTDIR/helloworld-service.yaml
echo "Waiting for mysql and redis startup before starting guestbook"
sleep 120
kubectl apply -f <(istioctl kube-inject -f $SCRIPTDIR/guestbook-deployment-v1.yaml) -f $SCRIPTDIR/guestbook-service.yaml
sleep 120
echo "Waiting for guestbook service startup before starting Guest Book UI"
kubectl apply -f <(istioctl kube-inject -f $SCRIPTDIR/ui-deployment.yaml) -f $SCRIPTDIR/guestbook-ui-service.yaml
